    ...
    <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
      {designers.map((designer, index) => {
        ...
        return (
          ...
          >
            <div className="relative aspect-3/4 overflow-hidden rounded-3xl bg-asoebi-mist shadow-[0_18px_55px_rgba(42,17,87,.10)]">
              <Image
                src={media.src}
                alt={`${designer.name}, Asoebi Fashion Week designer`}
                fill
                sizes="(min-width:1024px) 33vw, (min-width:768px) 50vw, 100vw"
                className={`transition-linear object-cover transition-transform duration-700 group-hover:scale-[1.025] ${media.portraitPosition}`}
              />
            </div>
            <div className="mt-5 flex items-start justify-between gap-5">
              ...
              <span
                aria-hidden="true"
                className="transition-linear mt-1 transition-transform group-hover:translate-x-1"
              >
                ↗
              </span>
            </div>
